...
- [Devlog \#3](https://github.com/lawrenceshi/ModsNGX/blob/main/Devlogs/3.md) (latest) (This Devlog)
...
It is kinda weird, but I commit my latest changes first, and then commit my early generator code. Its somehow out of order. Some code I commit now is written two weeks ago.

### Challenges:
I want to make everything to be customizable, but I also don't want to make the arguments  become overwhelming. At the same time, I want to make it simple to maintain and develop.

I also messed up and made some mistake when replacing the fixed paths with `ARG`, I really shouldn't smash them all in one commit. 
...
- ARG can be used in the `FROM image:tag` command, but it has to be declared before the **first** `FROM`, even if there are multiple `FROM` in the file.
...
- [50%] Support stable and fixed version (did fixed, didn't support the Nginx stable line)  
